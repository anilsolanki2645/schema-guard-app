...
def send_password_reset_email(email: str, code: str) -> bool:
    ...
    try:
        ...
        if res.returncode == 0:
            ...
            logger.error(f"Email subprocess failed with exit code {res.returncode}. Stderr: {res.stderr}")
            raise Exception(f"Subprocess exit code {res.returncode}. Stderr: {res.stderr}")
    except Exception as e:
        logger.error(f"Failed to run email subprocess: {e}")
        raise e
